...
{ 
 ...
 <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6"> 
            <h1 class="m-0 text-dark">Teachers borrowed information</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                      ...
                      if(isset($_SESSION['auth'])==$_SESSION['auth_user']){
                        $ot="SELECT admin.rol_as,email,issue_device.name,stdevice.Did,dev_name,dev_brand,issue,dev_return FROM admin 
                            INNER JOIN issue_device inner join stdevice inner join teachers ON  issue_device.Did=stdevice.Did and
                             admin.name=issue_device.name and admin.id=teachers.user WHERE issue_device.approve ='yes' ORDER BY `issue_device`.`dev_return` ASC";
